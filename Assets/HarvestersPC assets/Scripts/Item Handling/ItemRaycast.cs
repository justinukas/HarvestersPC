...
public class ItemRaycast : MonoBehaviour
{
    ...
    public void CheckRaycast(ref string currentTool, ref GameObject grabbedTool, ref string currentPlant, ref GameObject grabbedPlant)
    {
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (Physics.Raycast(ray, out RaycastHit hit, 1.5f, Grabbables))
            {
                ...
                {
                    GrabItem(ray, hit, ref currentTool, ref grabbedTool, currentPlant);
                }

                if (currentPlant == "null")
                {
                    GrabPlant(ray, hit, ref currentPlant, ref grabbedPlant);
                }
            }
    ...
    private void GrabItem(Ray ray, RaycastHit hit, ref string currentTool, ref GameObject grabbedTool, string currentPlant)
    {
    ...
    private void GrabPlant(Ray ray, RaycastHit hit, ref string currentPlant, ref GameObject grabbedPlant)
    {
